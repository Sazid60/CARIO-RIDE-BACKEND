...
export const generatePdf = async (invoiceData: IInvoiceData): Promise<Buffer> => {
    try {
        return new Promise((resolve, reject) => {
            ...
            // Header
            doc.font("Helvetica-Bold").fontSize(20).text("Ride Invoice", { align: "center" });
            doc.moveDown(1);
            ...
            // Ride Details
            doc.font("Helvetica-Bold").fontSize(14).text("Ride Details", { underline: true });
            doc.moveDown(0.5);

            const details = [
                { label: "Travel Distance", value: `${invoiceData.travelDistance} km` },
                { label: "Completed At", value: invoiceData.completedAt?.toLocaleString() || "N/A" },
            ];
            ...
            doc.moveDown(1);
            doc.font("Helvetica-Bold").fontSize(14).text(`Total Amount: $${invoiceData.totalFare.toFixed(2)}`, { align: "right" });
            doc.moveDown(2);
